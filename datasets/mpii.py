...
class MPII(data.Dataset):
    def __init__(self, opts, split):
        ...
        self.stuff1 = sio.loadmat(open(opts.worldCoors[:-4] + ('train' if split == 'train' else '') + '.mat', 'rb'))[
            'a']
        self.stuff2 = sio.loadmat(open(opts.headSize[:-4] + ('train' if split == 'train' else '') + '.mat', 'rb'))[
            'headSize']
